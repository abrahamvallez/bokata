...
**Execution constraint (critical):** Issue all three subagent reviews together, in parallel, within a single turn — never one at a time. Do not proceed, consolidate, or stop until every subagent has returned its result in this same exchange.
...
### 4a. Trio Contributes Rules from Their Lenses (Parallel Subagents)

**Execution constraint (critical):** Issue all three subagent contributions together, in parallel, within a single turn — never one at a time. Do not proceed, consolidate, or stop until every subagent has returned its result in this same exchange.

Spawn three subagents in parallel, each contributing Rules from their lens — they do NOT generate Scenarios themselves, only Rules:

**Subagent 1: PM Contribution**
```
...
**Subagent 2: Designer Contribution**
```
...
**Subagent 3: Engineer Contribution**
```
...
2. **Render & Write** `docs/initiatives/<initiative-slug>/backbone.html`:
   - Using the `html-template.md` conversion rules from the project's `.codex/resources/` directory (markdown → semantic HTML)
...
3. **Create & Render** `docs/initiatives/<initiative-slug>/story-map.html`:
   - Using the `story-map-template.md` from the project's `.codex/resources/` directory — **the addressable/column unit is the User Task (Step), not the Feature.** Do not build a `.map-column` per Feature with tasks listed inside it; that structure is wrong (see the template file's own warning about this).
...
2. **Render & Write** `docs/initiatives/<initiative-slug>/acceptance-criteria-functional.html`:
   - Using the `html-template.md` conversion rules from the project's `.codex/resources/` directory
