...
4. Create folder: `docs/initiatives/<initiative-slug>/<feature-slug>/` (use `mkdir -p`)
...
### 4a. Trio Contributes Rules Scoped per Skeleton Item / Increment (Parallel Subagents)
...
Spawn three subagents in parallel, each contributing Rules from their lens — scoped per Walking Skeleton item / per Increment:
