...
program
  ...
  .action(async (input, options) => {
    try {
      ...
      const result = await commands.bokata(projectInput);
      ...
      if (options.output) {
        ...
        const filename = `project-analysis-${new Date().toISOString().split('T')[0]}.md`;
...
program
  ...
  .action(async (input, options) => {
    try {
      ...
      const result = await commands.bokataFeature(featureInput);
      ...
      if (options.output) {
        const outputPath = path.resolve(options.output);
        fs.mkdirSync(outputPath, { recursive: true });
        const filename = `feature-analysis-${new Date().toISOString().split('T')[0]}.md`;
        const fullPath = path.join(outputPath, filename);
        fs.writeFileSync(fullPath, result.output);
        console.log(`\n📄 Saved to: ${fullPath}`);
      }
...
program
  ...
  .action(async (input, options) => {
    try {
      ...
      const result = await commands.bokataIterationsPaths(analysisInput);
      ...
      if (options.output) {
        const outputPath = path.resolve(options.output);
        fs.mkdirSync(outputPath, { recursive: true });
        const filename = `paths-${new Date().toISOString().split('T')[0]}.md`;
        const fullPath = path.join(outputPath, filename);
        fs.writeFileSync(fullPath, result.output);
        console.log(`\n📄 Saved to: ${fullPath}`);
      }
...
program
  ...
  .action(async (input, options) => {
    try {
      ...
      const result = await commands.bokataMatrix(analysisInput);
      ...
      if (options.output) {
        const outputPath = path.resolve(options.output);
        fs.mkdirSync(outputPath, { recursive: true });
        const filename = `matrix-${new Date().toISOString().split('T')[0]}.md`;
        const fullPath = path.join(outputPath, filename);
        fs.writeFileSync(fullPath, result.output);
        console.log(`\n📄 Saved to: ${fullPath}`);
      }
